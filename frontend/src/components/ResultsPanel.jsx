...
export default function ResultsPanel({ analysis, onReset }) {
  const { summary, strengths, improvements, missingKeywords, candidateName, alignment, recommendedRoles } = analysis;

  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-4">

      <motion.div variants={item} className="glass rounded-3xl p-5 sm:p-6">
        <h3 className="font-display text-xl font-semibold text-(--text-h)">Summary</h3>
        {candidateName && <p className="mt-1 text-sm font-medium text-(--text)/80">Prepared for {candidateName}</p>}
        <p className="mt-3 leading-relaxed text-(--text)/85">{summary}</p>
      </motion.div>

      <motion.div variants={item} className="glass rounded-3xl p-5 sm:p-6">
        <h3 className="font-display text-lg font-semibold text-teal-500">Strengths</h3>
        {strengths?.map((s, i) => (
      ...
      <motion.div variants={item} className="glass rounded-3xl p-5 sm:p-6">
        <h3 className="font-display text-lg font-semibold text-amber-500">Improvements</h3>
      ...
      <motion.div variants={item} className="glass rounded-3xl p-5 sm:p-6">
        <h3 className="font-display text-lg font-semibold text-sky-500">Missing Keywords</h3>
      ...
      {(alignment || (recommendedRoles && recommendedRoles.length)) && (
        <motion.div variants={item} className="glass rounded-3xl p-5 sm:p-6">
          <h3 className="font-display text-lg font-semibold text-(--text-h)">Role Alignment</h3>
          {alignment && (
            <p className="mt-2 text-(--text)/85">
              Matches target role:
              <strong className="ml-2 text-(--text-h)">{alignment.matches ? 'Yes' : 'No'}</strong>
              <span className="ml-1">({Math.round((alignment.confidence || 0) * 100)}% confidence)</span>
            </p>
          )}

          {recommendedRoles && recommendedRoles.length > 0 && (
            <div className="mt-3">
              <h4 className="mb-2 text-sm text-(--text)/80">Recommended roles</h4>
              <div className="flex flex-wrap gap-2">
                {recommendedRoles.map((r, i) => (
                  <span key={i} className="rounded-full border border-teal-400/30 bg-teal-500/10 px-3 py-1 text-xs text-teal-700 dark:text-teal-200">{r}</span>
                ))}
              </div>
            </div>
          )}
